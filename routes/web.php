...
Route::prefix('tessaban5')->group(function () {
    Route::get(
        "/",
        [VocabController::class, "index"]
    )->name("vocabs.index");
    Route::get(
        "vocabs/{grade}",
        [VocabController::class, "fetchVocabs"]
    )->name("vocabs.fetch");
    Route::put(
        "vocabs/{vocab}",
        [VocabController::class, "update"]
    )->name("vocabs.update");
    Route::post(
        "accept-cookie",
...
Route::post(
    ...
    [UserSetupController::class, 'sentResetPassword']
...
Route::prefix('dashboard')->middleware(['auth:sanctum', 'verified'])->group(function () {
    Route::middleware(["role:" . Role::ROLE_ADMIN . '|' . Role::ROLE_SUPER_ADMIN])->group(function () {
        ...
        Route::post(
            "schools/{school}/add-subject",
            [SchoolController::class, "addSubject"]
        )->name("dashboard.schools.add_subject");
        Route::post(
            "schools/{school}/delete-subject",
            [SchoolController::class, "deleteSubject"]
        )->name("dashboard.schools.delete_subject");
        Route::post(
            "schools/{school}/add-teacher",
            [SchoolController::class, "addTeacher"]
        )->name("dashboard.schools.add_teacher");
        Route::post(
            "schools/{school}/remove-teacher",
            [SchoolController::class, "removeTeacher"]
        )->name("dashboard.schools.remove_teacher");
        Route::post(
            "schools/{school}/bulk-lesson-plan-approve",
            [SchoolController::class, "bulkApprove"]
        )->name("dashboard.schools.bulk_approve");
        ...
        Route::get("schools/{school}/calendar", [SchoolController::class, "calendar"])
            ->name("dashboard.schools.calendar");
        ...
        )->name("dashboard.reports.approve");

        //clockins
        Route::get(
            "clock-ins",
            [ClockInController::class, "index"]
        )->name("dashboard.clock_ins.index");
        Route::post(
            "clock-ins/generate-report",
    ...
    Route::middleware(["role:" . Role::ROLE_ADMIN . '|' . Role::ROLE_SUPER_ADMIN . '|' . Role::ROLE_TEACHER])->group(
        function () {
            ...
            Route::post(
                "reports/{report}/next",
                [ReportController::class, "next"]
            )->name("dashboard.reports.next");
            Route::post(
                "reports/{report}/previous",
                [ReportController::class, "previous"]
            )->name("dashboard.reports.previous");
            Route::delete(
                "reports/{report}",
            ...
            Route::get(
                ...
                [ReportController::class, "printPreview"]
            )->name("dashboard.reports.print_preview");
            Route::post(
                "print-reports-generate-link",
                [ReportController::class, "generateLink"]
            )->name("dashboard.reports.generate_link");

            //clock in
            Route::post(
                "clock-in",
                [ClockInController::class, "in"]
            )->name("dashboard.clock_ins.in");
            Route::post(
                "clock-out",
                [ClockInController::class, "out"]
            )->name("dashboard.clock_ins.out");
...
Route::get(
    "global-reports/{globalReport}",
    [ReportController::class, "showGlobalReports"]
)->name("reports.global.show");
Route::get(
    "clock-ins/report-preview",
    [ClockInController::class, "clockInPreview"]
)->name("dashboard.clock_ins.preview");
